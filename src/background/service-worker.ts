...
function normalizeReport(raw: Record<string, unknown>): SandboxReportSW {
  ...
  const TRANSLATIONS: Record<string, string> = {
    'fetch': 'Envía datos a servidores externos (ej: para sincronizar tu progreso)',
    'innerHTML': 'Modifica el contenido de la página (necesario para mostrar la interfaz de la extensión)',
    'chrome.storage.local.set': 'Guarda información de la extensión en tu navegador',
    'chrome.storage.local.get': 'Lee información guardada anteriormente',
    'setInterval': 'Realiza tareas automáticas en segundo plano',
    'chrome.tabs.executeScript': 'Interactúa con el contenido de tus pestañas abiertas',
    'chrome.cookies.get': 'Accede a tus sesiones (cookies) para autenticación o personalización',
    'eval': 'Ejecuta código dinámico (puede ocultar comportamiento malicioso)',
    'keyboard': 'Detecta teclas presionadas (ej: para atajos o juegos)',
    'Reads page text content': 'Puede leer el texto de las páginas para ofrecer funcionalidades',
    'Registers keyboard event listeners': 'Responde a tus entradas de teclado',
    'Intercepts form submissions': 'Procesa la información que envías en formularios',
    'Monitors input events': 'Detecta cuando escribes en tiempo real',
